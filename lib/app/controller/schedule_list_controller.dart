  ...
  List<SubjectBO> _calculateSubjectToUpload(ScheduleBO schedule) {
    ...
    for (var itemA in schedule.subjects) {
      if (initialSubjects.any((element) => element.id == itemA?.id)) {
        ...
        subjectsToAdd.add(SubjectBO(subject.name, subject.acronym, subject.classGroup, subject.seminary, subject.laboratory, subject.english, subject.time - 30 * count, subject.semester, subject.classroom, subject.department, subject.degree, subject.color, subject.id));
      }
    }
